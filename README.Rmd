...
genomic read aligner, with R through a packaged CLI, native C bindings, and a
pinned vendored source tree with patch provenance.
...
Rminibwa builds and installs its own `minibwa` executable from the vendored
source. The native C-library backend uses GNU make and zlib.
...
columns directly. The same source is displayed here and compiled in the next
chunk.

```{r downstream-c-code, echo = FALSE, results = "asis"}
capi_path <- file.path("inst", "capi", "rminibwa_tinycc_consumer.c")
if (!file.exists(capi_path)) {
...
Python/Rust comparison artifacts and renders these opt-in developer benchmarks.
The workload uses a chrM-sized random reference and an indel-mutated long read.
...
Rminibwa itself installs a package-built `minibwa` executable during package
installation.
